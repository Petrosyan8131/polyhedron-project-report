...
class Polyedr:
    ...
    def __init__(self, file):
        ...
        # начальная сумма длин рёбер
        self.s = 0.0

        # списки вершин, рёбер и граней полиэдра
        self.vertexes, self.edges, self.facets = [], [], []
        self.vertst, self.edgest = [], []
        ...
        with open(file) as f:
            for i, line in enumerate(f):
                if i == 0:
                    ...
                    vertst = list(self.vertst[int(n) - 1] for n in buf)
                    # задание рёбер грани
                    for n in range(size):
                        self.edges.append(Edge(vertexes[n - 1], vertexes[n]))
                        self.edgest.append(Edge(vertst[n - 1], vertst[n]))
                    # задание самой грани
                    self.facets.append(Facet(vertexes))

# Удаление дубликатов рёбер и подсчёт суммы длин
